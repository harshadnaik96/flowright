...
### `onProgress` callback and live heal log

`healSelector` accepts an optional `onProgress?: (message: string) => void` callback. The runner passes a closure that broadcasts `step:heal-log` WS events for each phase:

```ts
await healSelector(page, step, runId, db, {
  onProgress: (msg) => broadcast(runId, {
    type: "step:heal-log",
    payload: { stepOrder: step.order, healLogMessage: msg },
  }),
})
```

Four messages are emitted per heal attempt:
1. `"Scanning live DOM…"` — before `extractElements`
2. `"Found N elements (Xms)"` — after extraction, includes count and wall-clock latency
3. `"Asking Gemini for a fix…"` — before Gemini call
4. Either `"Proposal: <healedCommand>"` or `"No proposal returned"` — after Gemini responds

If extraction itself fails, `"DOM extraction failed"` is emitted instead of messages 2–4.
...
```ts
// step:heal-log — emitted progressively during a heal attempt (multiple per heal)
{ type: "step:heal-log", runId, payload: {
    stepOrder: number,
    healLogMessage: string,   // e.g. "Scanning live DOM…", "Found 47 elements (230ms)", "Asking Gemini for fix…", "Proposal: cy.get('[data-testid=\"signin\"]').click()"
}}
...
The run viewer listens for `step:heal-log` and renders each message into a live **AI Heal Activity** terminal log shown beneath the failing step row. The messages arrive in real-time as each phase completes:

1. `"Scanning live DOM…"` — extractElements started
2. `"Found N elements (Xms)"` — extraction complete with element count and latency
3. `"Asking Gemini for a fix…"` — Gemini request in flight
4. `"Proposal: <healedCommand>"` or `"No proposal returned"` — Gemini result

The log is rendered as a dark terminal block (`bg-black/80`) with monospace yellow text so it's readable in both light and dark themes.

`step:healed` arrives after `step:heal-log` messages, and the run viewer renders a yellow `healed` badge on the step row with the selector diff in the tooltip. The badge persists into the final state if the step ultimately passes.
