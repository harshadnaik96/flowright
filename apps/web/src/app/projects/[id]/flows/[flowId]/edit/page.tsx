...
export default function EditFlowPage({
  ...
  // Form state — pre-filled from existing flow
  ...
  // ── Step 2: Re-generate steps for existing flow ────────────────────────────
  async function handleGenerate() {
    setIsGenerating(true)
    setError(null)
    try {
      const result = await api.generator.regenerateFlow(flowId, {
  ...
  function handleVariableChange(key: string, value: string) {
    setVariables((prev) => prev.map((v) => (v.key === key ? { ...v, defaultValue: value } : v)))
  }

  const panelLabels: Record<Panel, string> = { write: "Write", refine: "Refine", steps: "Review" }

  if (loading) return <AppShell><div className="max-w-2xl mx-auto text-sm text-muted-foreground">Loading…</div></AppShell>

  return (
    <AppShell>
      <div className="max-w-2xl mx-auto">
        <p className="text-xs text-muted-foreground mb-6">
          Editing <span className="font-medium text-foreground">{flowName}</span> — regenerating will clear run history
        </p>
        ...
        <div className="flex items-center gap-2 mb-8">
          {(["write", "refine", "steps"] as Panel[]).map((p, i) => (
            <div key={p} className="flex items-center gap-2">
              ...
              <span className={`text-sm ${p === panel ? "font-medium" : "text-muted-foreground"}`}>
                {panelLabels[p]}
              </span>
